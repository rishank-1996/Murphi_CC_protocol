...
HomeState:
  Record
    state: enum {
      H_Invalid, H_Shared, H_Modified, H_SD,   --stable states
      H_SA, H_MA          --transient
    };
    owner: Node;
...
ProcState:
  Record
    state: enum{
        P_Invalid, P_Shared, P_Modified,                --stable states
        ISD, IMAD, IMA, SMAD, SMA, MIA, SIA, IIA      --transient states
        };
    val: Value;
    cnt: CountType;
    totalacks: CountType;
...
Begin
  ...
  case ISD:
    ...
    case ackdata:
      Send(Sack, HomeType, p, VC1, msg.val, p, UNDEFINED);
      ps := P_Shared;
      pv := msg.val;
    else
      ErrorUnhandledMsg(msg, p);
  ...
  case P_Modified:
    ...
    case FwdGetS:
      ...
      --Send(ackdata, HomeType, p, VC1, pv, p, UNDEFINED);
      ps := P_Shared;
    case FwdGetM:
      Send(ackdata, msg.fwd, p, VC1, pv, p, UNDEFINED);
      ps := P_Invalid;
