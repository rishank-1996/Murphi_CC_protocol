/******************************
  ...
  Murphi Last Compiled date: "Mar  3 2020"
 ******************************/
...
class mu_1_MessageType: public mu__byte
{
 public:
  ...
  mu_1_MessageType (char *name, int os): mu__byte(7, 23, 5, name, os) {};
  mu_1_MessageType (void): mu__byte(7, 23, 5) {};
  mu_1_MessageType (int val): mu__byte(7, 23, 5, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1_MessageType::values[] = {"data","ackdata","GetS","PutS","GetM","PutM","ackCount","PutO","PutAck","PutAckM","OData","Sack","Mack","FwdGetM","FwdGetS","Invreq","InvAck",NULL };
...
void mu_1_Message::set_self(char *n, int os)
{
  ...
  mu_dst.set_self_2(name, ".dst", os + 8 );
  mu_src.set_self_2(name, ".src", os + 16 );
  mu_vc.set_self_2(name, ".vc", os + 24 );
  mu_val.set_self_2(name, ".val", os + 32 );
  mu_fwd.set_self_2(name, ".fwd", os + 40 );
  mu_cnt.set_self_2(name, ".cnt", os + 48 );
}
...
class mu_1__type_0: public mu__byte
{
 public:
  ...
  mu_1__type_0 (char *name, int os): mu__byte(24, 30, 3, name, os) {};
  mu_1__type_0 (void): mu__byte(24, 30, 3) {};
  mu_1__type_0 (int val): mu__byte(24, 30, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_0::values[] = {"H_Invalid","H_Shared","H_Modified","H_Owner","H_SA","H_MA","H_OA",NULL };
...
class mu_1__type_2: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1__type_2::values[ int(val) - 31] );
    else return ( s << "Undefined" );
  };

  mu_1__type_2 (char *name, int os): mu__byte(31, 45, 4, name, os) {};
  mu_1__type_2 (void): mu__byte(31, 45, 4) {};
  mu_1__type_2 (int val): mu__byte(31, 45, 4, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
char *mu_1__type_2::values[] = {"P_Invalid","P_Shared","P_Modified","P_Owner","ISD","IMAD","IMA","SMAD","SMA","MIA","OMAC","OMA","OIA","SIA","IIA",NULL };
...
friend int CompareWeight(mu_1_ProcState& a, mu_1_ProcState& b)
  {
    ...
    w = CompareWeight(a.mu_cnt, b.mu_cnt);
    if (w!=0) return w;
    w = CompareWeight(a.mu_totalacks, b.mu_totalacks);
    if (w!=0) return w;
  return 0;
}
friend int Compare(mu_1_ProcState& a, mu_1_ProcState& b)
  {
    ...
    w = Compare(a.mu_cnt, b.mu_cnt);
    if (w!=0) return w;
    w = Compare(a.mu_totalacks, b.mu_totalacks);
    if (w!=0) return w;
  return 0;
}
  virtual void Permute(PermSet& Perm, int i);
  ...
  void print_diff(state *prevstate) {
    ...
    mu_val.print_diff(prevstate);
    mu_cnt.print_diff(prevstate);
    mu_totalacks.print_diff(prevstate);
  };
  void to_state(state *thestate) {
    mu_state.to_state(thestate);
    mu_val.to_state(thestate);
    mu_cnt.to_state(thestate);
    mu_totalacks.to_state(thestate);
  };
virtual bool isundefined() { Error.Error("Checking undefinedness of a non-base type"); return TRUE;}
virtual bool ismember() { Error.Error("Checking membership for a non-base type"); return TRUE;}
  mu_1_ProcState& operator= (const mu_1_ProcState& from) {
    ...
    mu_val.value(from.mu_val.value());
    mu_cnt.value(from.mu_cnt.value());
    mu_totalacks.value(from.mu_totalacks.value());
    return *this;
  };
};
...
void mu_1_ProcState::set_self(char *n, int os)
{
  ...
  mu_totalacks.set_self_2(name, ".totalacks", os + 24 );
}
...
class mu_1__type_4_id: public mu__byte
{
 public:
  ...
  inline operator int() const { return value(); };
  mu_1__type_4_id () : mu__byte(0,102,0) {};
  mu_1__type_4_id (int val) : mu__byte(0,102,0, "Parameter or function result.",0) {operator=(val); };
  char * Name() { return tsprintf("%d", value()); };
};
class mu_1__type_4
{
 public:
  mu_1_Message array[ 103 ];
  int max_size;
  int current_size;
 public:
  mu_0_boolean valid[ 103 ];
  char *name;
  char longname[BUFFER_SIZE/4];
  void set_self( char *n, int os);
  void set_self_2( char *n, char *n2, int os);
  ...
  {
    if ((index >= 0) && (index <= 102) && valid[index].value())
      return array[ index ];
    else {
      Error.Error("Internal Error::%d not in index range of %s.", index, name);
      return array[0];
...
friend int Compare(mu_1__type_4& a, mu_1__type_4& b)
  ...
  virtual void Limit(PermSet& Perm);
  virtual void MultisetLimit(PermSet& Perm);
  void clear() { for (int i = 0; i < 103; i++) { array[i].undefine(); valid[i].value(FALSE); } current_size = 0; };

  void undefine() { for (int i = 0; i < 103; i++) { array[i].undefine(); valid[i].value(FALSE); } current_size = 0; };

  void reset() { for (int i = 0; i < 103; i++) { array[i].undefine(); valid[i].value(FALSE); } current_size = 0; };
  ...
  {
    update_size();
    if (current_size >= 103) Error.Error("Maximum size of MultiSet (%s) exceeded.",name);
    int i;
    for (i = 0; i < 103; i++)
      if (!valid[i].value())
        {
...
void mu_1__type_4::set_self( char *n, int os)
{
  ...
  name = n;
  for(i = 0; i < 103; i++)
    array[i].set_self(tsprintf("%s{%d}", n,i), i * 56 + os);
  k = os + i * 56;
  for(i = 0; i < 103; i++)
    valid[i].set_self("", i * 8 + k);
};
...
void mu_1__type_5::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"HomeType", i * 6592 + os);i++;
array[i].set_self_ar(n,"Proc_1", i * 6592 + os);i++;
array[i].set_self_ar(n,"Proc_2", i * 6592 + os);i++;
array[i].set_self_ar(n,"Proc_3", i * 6592 + os);i++;
}
...
void mu_1__type_7::set_self( char *n, int os)
  {
    int i=0;
    name = n;
array[i].set_self_ar(n,"HomeType", i * 168 + os);i++;
array[i].set_self_ar(n,"Proc_1", i * 168 + os);i++;
array[i].set_self_ar(n,"Proc_2", i * 168 + os);i++;
array[i].set_self_ar(n,"Proc_3", i * 168 + os);i++;
}
...
const int mu_data = 7;
const int mu_ackdata = 8;
const int mu_GetS = 9;
const int mu_PutS = 10;
const int mu_GetM = 11;
const int mu_PutM = 12;
const int mu_ackCount = 13;
const int mu_PutO = 14;
const int mu_PutAck = 15;
const int mu_PutAckM = 16;
const int mu_OData = 17;
const int mu_Sack = 18;
const int mu_Mack = 19;
const int mu_FwdGetM = 20;
const int mu_FwdGetS = 21;
const int mu_Invreq = 22;
const int mu_InvAck = 23;
const int mu_H_Invalid = 24;
const int mu_H_Shared = 25;
const int mu_H_Modified = 26;
const int mu_H_Owner = 27;
const int mu_H_SA = 28;
const int mu_H_MA = 29;
const int mu_H_OA = 30;
const int mu_P_Invalid = 31;
const int mu_P_Shared = 32;
const int mu_P_Modified = 33;
const int mu_P_Owner = 34;
const int mu_ISD = 35;
const int mu_IMAD = 36;
const int mu_IMA = 37;
const int mu_SMAD = 38;
const int mu_SMA = 39;
const int mu_MIA = 40;
const int mu_OMAC = 41;
const int mu_OMA = 42;
const int mu_OIA = 43;
const int mu_SIA = 44;
const int mu_IIA = 45;
...
/*** Variable declaration ***/
mu_1__type_5 mu_Net("Net",168);

/*** Variable declaration ***/
mu_1__type_7 mu_InBox("InBox",26536);

/*** Variable declaration ***/
mu_0_boolean mu_msg_processed("msg_processed",27208);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",27216);

void mu_Send(const mu_1_MessageType& mu_mtype, const mu_1_Node& mu_dst, const mu_1_Node& mu_src, const mu_1_VCType& mu_vc, const mu_1_Value& mu_val, const mu_1_Node& mu_fwd, const mu_1_CountType& mu_cnt)
{
/*** Variable declaration ***/
...
bool mu__boolexpr12;
  if (!((mu_n>=1 && mu_n<=3))) mu__boolexpr12 = FALSE ;
  else {
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_HomeNode.mu_sharers.valid[(int)mu_i].value())
        {
          if ( (mu_HomeNode.mu_sharers[mu_i]) == (mu_n) ) mu__intexpr13++;
        }
      if (mu_i == 3-1) break;
    }
  }
/*** end multisetcount 2 declaration ***/
  mu__boolexpr12 = ((mu__intexpr13) != (0)) ; 
}
if ( mu__boolexpr12 )
{
/*** end multisetremove 0 declaration ***/
  mu_1__type_1_id mu__idexpr14;
  mu_1__type_1_id mu_i;
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_HomeNode.mu_sharers.valid[(int)mu_i].value())
        {
          if ( (mu_HomeNode.mu_sharers[mu_i]) == (mu_n) ) { mu__idexpr14 = mu_i; mu_HomeNode.mu_sharers.multisetremove(mu__idexpr14); };
        }
      if (mu_i == 3-1) break;
    }
/*** end multisetremove 0 declaration ***/
...
/*** end multisetremove 1 declaration ***/
  mu_1__type_1_id mu__idexpr16;
  mu_1__type_1_id mu_i;
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_HomeNode.mu_sharers.valid[(int)mu_i].value())
        {
          if ( (mu_HomeNode.mu_sharers[mu_i]) == (mu_n) ) { mu__idexpr16 = mu_i; mu_HomeNode.mu_sharers.multisetremove(mu__idexpr16); };
        }
      if (mu_i == 3-1) break;
    }
/*** end multisetremove 1 declaration ***/
...
if ( (mu_n) != (mu_rqst) )
{
mu_Send ( mu_Invreq, mu_n, (int)mu_HomeType, mu_VC1, mu_1_Value_undefined_var, mu_rqst, mu_1_CountType_undefined_var );
}
}
};
};
...
mu_cnt = mu__intexpr19;
mu_msg_processed = mu_true;
switch ((int) mu_HomeNode.mu_state) {
case mu_H_Invalid:
switch ((int) mu_msg.mu_mtype) {
case mu_GetS:
mu_HomeNode.mu_state = mu_H_SA;
mu_Send ( mu_ackdata, mu_msg.mu_src, (int)mu_HomeType, mu_VC0, mu_HomeNode.mu_val, mu_msg.mu_src, mu_1_CountType_undefined_var );
mu_AddToSharersList ( mu_msg.mu_src );
break;
case mu_GetM:
mu_HomeNode.mu_state = mu_H_MA;
mu_HomeNode.mu_owner = mu_msg.mu_src;
mu_Send ( mu_ackCount, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val, mu_msg.mu_src, (int)mu_cnt );
break;
case mu_PutS:
mu_Send ( mu_PutAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC0, mu_1_Value_undefined_var, mu_msg.mu_src, mu_1_CountType_undefined_var );
break;
case mu_PutM:
if ( (mu_HomeNode.mu_owner) != (mu_msg.mu_src) )
{
mu_Send ( mu_PutAck, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_1_Value_undefined_var, mu_msg.mu_src, mu_1_CountType_undefined_var );
}
break;
case mu_PutO:
if ( (mu_HomeNode.mu_owner) != (mu_msg.mu_src) )
...
case mu_GetM:
bool mu__boolexpr20;
  if (!((mu_cnt) == (1))) mu__boolexpr20 = FALSE ;
  else {
  mu__boolexpr20 = (mu_IsSharer( mu_msg.mu_src )) ; 
}
if ( mu__boolexpr20 )
{
mu_Send ( mu_ackCount, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val, mu_msg.mu_src, 0 );
}
else
{
if ( mu_IsSharer( mu_msg.mu_src ) )
{
mu_Send ( mu_ackCount, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val, mu_msg.mu_src, (mu_cnt) - (1) );
}
else
{
mu_Send ( mu_ackCount, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val, mu_msg.mu_src, (int)mu_cnt );
...
switch ((int) mu_msg.mu_mtype) {
case mu_GetS:
mu_Send ( mu_FwdGetS, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC0, mu_1_Value_undefined_var, mu_msg.mu_src, mu_1_CountType_undefined_var );
mu_AddToSharersList ( mu_msg.mu_src );
mu_HomeNode.mu_state = mu_H_OA;
break;
case mu_GetM:
if ( (mu_HomeNode.mu_owner) != (mu_msg.mu_src) )
{
mu_Send ( mu_FwdGetM, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC1, mu_1_Value_undefined_var, mu_msg.mu_src, mu_1_CountType_undefined_var );
if ( mu_IsSharer( mu_msg.mu_src ) )
{
mu_Send ( mu_ackCount, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_HomeNode.mu_val, mu_msg.mu_src, (int)mu_cnt );
}
...
{
mu_Send ( mu_ackCount, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_1_Value_undefined_var, mu_msg.mu_src, (int)mu_cnt );
}
mu_HomeNode.mu_state = mu_H_MA;
mu_SendInvReqToSharers ( mu_msg.mu_src );
mu_ClearSharersList (  );
...
break;
case mu_GetM:
mu_Send ( mu_OData, mu_HomeNode.mu_owner, (int)mu_HomeType, mu_VC0, mu_1_Value_undefined_var, mu_msg.mu_src, mu_1_CountType_undefined_var );
mu_HomeNode.mu_owner = mu_msg.mu_src;
mu_HomeNode.mu_state = mu_H_MA;
break;
case mu_PutS:
...
if ( (mu_HomeNode.mu_owner) != (mu_msg.mu_src) )
{
mu_Send ( mu_OData, mu_msg.mu_src, (int)mu_HomeType, mu_VC1, mu_1_Value_undefined_var, mu_HomeNode.mu_owner, mu_1_CountType_undefined_var );
}
else
{
mu_HomeNode.mu_state = mu_H_Invalid;
...
switch ((int) mu_msg.mu_mtype) {
case mu_Sack:
mu_HomeNode.mu_state = mu_H_Owner;
break;
case mu_GetM:
mu_msg_processed = mu_false;
break;
...
{
/*** Variable declaration ***/
mu_1__type_9 mu_ncount("ncount",0);
...
{
  mu_1_CountType& mu_pc = mu_Procs[mu_p].mu_cnt;
{
  mu_1_CountType& mu_pt = mu_Procs[mu_p].mu_totalacks;
switch ((int) mu_ps) {
case mu_P_Invalid:
switch ((int) mu_msg.mu_mtype) {
case mu_Invreq:
...
break;
case mu_ackdata:
mu_Send ( mu_Sack, (int)mu_HomeType, (int)mu_p, mu_VC1, mu_1_Value_undefined_var, (int)mu_p, mu_1_CountType_undefined_var );
mu_ps = mu_P_Shared;
mu_pv = mu_msg.mu_val;
break;
default:
...
mu_pv = mu_msg.mu_val;
mu_ps = mu_P_Modified;
mu_Send ( mu_Mack, (int)mu_HomeType, (int)mu_p, mu_VC1, mu_1_Value_undefined_var, (int)mu_p, mu_1_CountType_undefined_var );
mu_pt = 3;
mu_pc = 0;
}
else
...
case mu_Invreq:
mu_ps = mu_P_Invalid;
mu_Send ( mu_ackdata, mu_msg.mu_fwd, (int)mu_p, mu_VC1, mu_pv, (int)mu_p, mu_1_CountType_undefined_var );
mu_pv.undefine();
break;
default:
mu_ErrorUnhandledMsg ( mu_msg, (int)mu_p );
...
break;
case mu_Invreq:
mu_Send ( mu_ackdata, mu_msg.mu_fwd, (int)mu_p, mu_VC1, mu_msg.mu_val, (int)mu_p, mu_1_CountType_undefined_var );
break;
case mu_InvAck:
break;
case mu_ackdata:
...
switch ((int) mu_msg.mu_mtype) {
case mu_FwdGetS:
mu_Send ( mu_ackdata, mu_msg.mu_fwd, (int)mu_p, mu_VC0, mu_pv, (int)mu_p, mu_1_CountType_undefined_var );
mu_ps = mu_P_Owner;
break;
case mu_FwdGetM:
mu_Send ( mu_ackdata, mu_msg.mu_fwd, (int)mu_p, mu_VC1, mu_pv, (int)mu_p, mu_1_CountType_undefined_var );
...
break;
case mu_OData:
mu_Send ( mu_ackCount, mu_msg.mu_fwd, (int)mu_p, mu_VC1, mu_pv, (int)mu_p, 0 );
mu_ps = mu_P_Invalid;
mu_pv.undefine();
break;
case mu_ackdata:
...
public:
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_n.unionassign(r % 4);
    r = r / 4;
    while (what_rule < 424 && mu_midx.value()<103 )
      {
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("writeback from O, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  return (mu_p.mu_state) == (mu_P_Owner);
  }
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_PutO, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_p.mu_val, (int)mu_n, mu_1_CountType_undefined_var );
mu_p.mu_state = mu_OIA;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("writeback from M, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_PutM, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_p.mu_val, (int)mu_n, mu_1_CountType_undefined_var );
mu_p.mu_state = mu_MIA;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("writeback from S, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_PutS, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_1_Value_undefined_var, (int)mu_n, mu_1_CountType_undefined_var );
mu_p.mu_state = mu_SIA;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("store in s, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_GetM, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_1_Value_undefined_var, (int)mu_n, mu_1_CountType_undefined_var );
mu_p.mu_state = mu_SMAD;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("store in I, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("load in I, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
  ...
  void Code(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ProcState& mu_p = mu_Procs[mu_n];
mu_Send ( mu_GetS, (int)mu_HomeType, (int)mu_n, mu_VC0, mu_1_Value_undefined_var, (int)mu_n, mu_1_CountType_undefined_var );
mu_p.mu_state = mu_ISD;
  };
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("store in O, n:%s", mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Proc mu_n;
...
public:
  ...
  char * Name(unsigned r)
  {
    ...
    mu_n.value((r % 3) + 1);
    r = r / 3;
    return tsprintf("store in M, v:%s, n:%s", mu_v.Name(), mu_n.Name());
  }
  bool Condition(unsigned r)
  {
    static mu_1_Value mu_v;
...
void SetNextEnabledRule(unsigned & what_rule)
{
  ...
  if (what_rule>=12 && what_rule<424)
    { R1.NextRule(what_rule);
      if (what_rule<424) return; }
  if (what_rule>=424 && what_rule<427)
    { R2.NextRule(what_rule);
      if (what_rule<427) return; }
  if (what_rule>=427 && what_rule<430)
    { R3.NextRule(what_rule);
      if (what_rule<430) return; }
  if (what_rule>=430 && what_rule<433)
    { R4.NextRule(what_rule);
      if (what_rule<433) return; }
  if (what_rule>=433 && what_rule<436)
    { R5.NextRule(what_rule);
      if (what_rule<436) return; }
  if (what_rule>=436 && what_rule<439)
    { R6.NextRule(what_rule);
      if (what_rule<439) return; }
  if (what_rule>=439 && what_rule<442)
    { R7.NextRule(what_rule);
      if (what_rule<442) return; }
  if (what_rule>=442 && what_rule<445)
    { R8.NextRule(what_rule);
      if (what_rule<445) return; }
  if (what_rule>=445 && what_rule<451)
...
bool Condition(unsigned r)
{
  ...
  if (r>=12 && r<=423) return R1.Condition(r-12);
  if (r>=424 && r<=426) return R2.Condition(r-424);
  if (r>=427 && r<=429) return R3.Condition(r-427);
  if (r>=430 && r<=432) return R4.Condition(r-430);
  if (r>=433 && r<=435) return R5.Condition(r-433);
  if (r>=436 && r<=438) return R6.Condition(r-436);
  if (r>=439 && r<=441) return R7.Condition(r-439);
  if (r>=442 && r<=444) return R8.Condition(r-442);
  if (r>=445 && r<=450) return R9.Condition(r-445);
Error.Notrace("Internal: NextStateGenerator -- checking condition for nonexisting rule.");
}
void Code(unsigned r)
{
  if (r<=11) { R0.Code(r-0); return; } 
  if (r>=12 && r<=423) { R1.Code(r-12); return; } 
  if (r>=424 && r<=426) { R2.Code(r-424); return; } 
  if (r>=427 && r<=429) { R3.Code(r-427); return; } 
  if (r>=430 && r<=432) { R4.Code(r-430); return; } 
  if (r>=433 && r<=435) { R5.Code(r-433); return; } 
  if (r>=436 && r<=438) { R6.Code(r-436); return; } 
  if (r>=439 && r<=441) { R7.Code(r-439); return; } 
  if (r>=442 && r<=444) { R8.Code(r-442); return; } 
  if (r>=445 && r<=450) { R9.Code(r-445); return; } 
}
int Priority(unsigned short r)
{
  if (r<=11) { return R0.Priority(); } 
  if (r>=12 && r<=423) { return R1.Priority(); } 
  if (r>=424 && r<=426) { return R2.Priority(); } 
  if (r>=427 && r<=429) { return R3.Priority(); } 
  if (r>=430 && r<=432) { return R4.Priority(); } 
  if (r>=433 && r<=435) { return R5.Priority(); } 
  if (r>=436 && r<=438) { return R6.Priority(); } 
  if (r>=439 && r<=441) { return R7.Priority(); } 
  if (r>=442 && r<=444) { return R8.Priority(); } 
  if (r>=445 && r<=450) { return R9.Priority(); } 
}
char * Name(unsigned r)
{
  if (r<=11) return R0.Name(r-0);
  if (r>=12 && r<=423) return R1.Name(r-12);
  if (r>=424 && r<=426) return R2.Name(r-424);
  if (r>=427 && r<=429) return R3.Name(r-427);
  if (r>=430 && r<=432) return R4.Name(r-430);
  if (r>=433 && r<=435) return R5.Name(r-433);
  if (r>=436 && r<=438) return R6.Name(r-436);
  if (r>=439 && r<=441) return R7.Name(r-439);
  if (r>=442 && r<=444) return R8.Name(r-442);
  if (r>=445 && r<=450) return R9.Name(r-445);
  return NULL;
}
};
const unsigned numrules = 451;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 451
...
/********************
  Invariant records
 ********************/
int mu__invariant_21() // Invariant "invalid means value doesn't exist in proc"
{
bool mu__quant22; 
mu__quant22 = TRUE;
{
...
bool mu__boolexpr23;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_Invalid))) mu__boolexpr23 = TRUE ;
  else {
  mu__boolexpr23 = (mu_Procs[mu_n].mu_val.isundefined()) ; 
}
if ( !(mu__boolexpr23) )
...
bool mu__condition_24() // Condition for Rule "invalid means value doesn't exist in proc"
{
...
int mu__invariant_30() // Invariant "values in memory matches value of last write, when shared or invalid"
{
bool mu__quant31; 
mu__quant31 = TRUE;
{
...
bool mu__boolexpr32;
bool mu__boolexpr33;
  if ((mu_HomeNode.mu_state) == (mu_H_Shared)) mu__boolexpr33 = TRUE ;
  else {
  mu__boolexpr33 = ((mu_HomeNode.mu_state) == (mu_H_Invalid)) ; 
}
  if (!(mu__boolexpr33)) mu__boolexpr32 = TRUE ;
  else {
  mu__boolexpr32 = ((mu_HomeNode.mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr32) )
...
bool mu__condition_34() // Condition for Rule "values in memory matches value of last write, when shared or invalid"
{
...
int mu__invariant_35() // Invariant "Invalid implies empty sharer list"
{
bool mu__boolexpr36;
...
bool mu__condition_38() // Condition for Rule "Invalid implies empty sharer list"
{
...
int mu__invariant_39() // Invariant "modified implies empty sharers list"
{
bool mu__boolexpr40;
  if (!((mu_HomeNode.mu_state) == (mu_H_Modified))) mu__boolexpr40 = TRUE ;
  else {
...
bool mu__condition_42() // Condition for Rule "modified implies empty sharers list"
{
...
{
bool mu__boolexpr44;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr44 = TRUE ;
  else {
...
{
bool mu__boolexpr47;
  if (!((mu_HomeNode.mu_state) == (mu_H_Invalid))) mu__boolexpr47 = TRUE ;
  else {
...
{"Invalid implies empty owner", &mu__condition_48, NULL, FALSE},
{"value in memory matches value of last write, when invalid", &mu__condition_45, NULL, FALSE},
{"modified implies empty sharers list", &mu__condition_42, NULL, FALSE},
{"Invalid implies empty sharer list", &mu__condition_38, NULL, FALSE},
{"values in memory matches value of last write, when shared or invalid", &mu__condition_34, NULL, FALSE},
{"values in shared state match memory", &mu__condition_29, NULL, FALSE},
{"invalid means value doesn't exist in proc", &mu__condition_24, NULL, FALSE},
};
...
void mu_1__type_4::MultisetLimit(PermSet& Perm)
{
  ...
  bool goodset_mu_1_Proc[3];
  int count_multisetindex, oldcount_multisetindex;
  bool pos_multisetindex[103][103];
  bool goodset_multisetindex[103];
  mu_1_Message temp;
                ...
                for (j=0; j<count_mu_1_Proc; j++) //scan through element priority
                  {
                    ...
                    for (k=0; k<current_size; k++) // scan array index
                      ...
                      if (pos_multisetindex[i][k] 
                          ...
                          && (*this)[k].mu_dst>=1
                          && (*this)[k].mu_dst<=3
                          && pos_mu_1_Proc[j][(*this)[k].mu_dst-1])
                        {
                          exists = TRUE;
                          goodset_multisetindex[k] = TRUE;
                          goodset_mu_1_Proc[(*this)[k].mu_dst-1] = TRUE;
                        }
                    if (exists)
                      {
                        // set split for the array index type
  ...
  if (Perm.MTO_class_mu_1_Proc() && count_mu_1_Proc<3)
    {
      ...
      for (k=0; k<3; k++) // step through class
        if ((*this)[k+1][i0].mu_dst.isundefined()
            ||(*this)[k+1][i0].mu_dst!=k+1)
          exists = TRUE;
        else
          split = TRUE;
      if (exists && split)
          ...
          for (i=0; i<count_mu_1_Proc; i++) // for value priority
            {
              ...
              for (k=0; k<3; k++) // step through class
                if (pos_mu_1_Proc[i][k] 
                    && !(*this)[k+1][i0].mu_dst.isundefined()
                    && (*this)[k+1][i0].mu_dst==k+1)
                  {
                    exists = TRUE;
                    goodset_mu_1_Proc[k] = TRUE;
      ...
      if (Perm.MTO_class_mu_1_Proc() && count_mu_1_Proc<3)
        {
          ...
          for (k=0; k<3; k++) // step through class
            if (!(*this)[k+1][i0].mu_dst.isundefined()
                &&(*this)[k+1][i0].mu_dst!=k+1
                &&(*this)[k+1][i0].mu_dst>=1
                &&(*this)[k+1][i0].mu_dst<=3)
              exists = TRUE;
          if (exists)
            {
              for (i=0; i<count_mu_1_Proc; i++) // for value priority
                  ...
                  for (j=0; j<count_mu_1_Proc; j++) // for value priority
                    {
                      ...
                      for (k=0; k<3; k++) // step through class
                        if (pos_mu_1_Proc[i][k] 
                            ...
                            && (*this)[k+1][i0].mu_dst>=1
                            && (*this)[k+1][i0].mu_dst<=3
                            && pos_mu_1_Proc[j][(*this)[k+1][i0].mu_dst-1])
                          {
                            exists = TRUE;
                            goodset_mu_1_Proc[k] = TRUE;
                          }
